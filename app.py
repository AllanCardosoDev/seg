...
st.markdown("""
    <style>
        ...
        .tbl-cbc { width:100%; border-collapse:collapse; font-size:0.84rem; }
        .tbl-cbc th {
            background-color:#FFC000; color:#000; font-weight:bold;
            padding:6px 10px; border:1px solid #d4a000;
        }
        .tbl-cbc th:last-child { text-align:right; }
        .tbl-cbc td {
            background-color:#FFC000; color:#000;
            padding:4px 10px; border:1px solid #d4a000;
        }
        .tbl-cbc td:last-child { text-align:right; }
        .tbl-cbc .total-row td {
            font-weight:bold; border-top:2px solid #a07800;
            background-color:#e6ac00;
        }
        .tbl-cbi { width:100%; border-collapse:collapse; font-size:0.84rem; }
        .tbl-cbi th {
            background-color:#92D050; color:#000; font-weight:bold;
            padding:6px 10px; border:1px solid #6aaa28;
        }
        .tbl-cbi th:last-child { text-align:right; }
        .tbl-cbi td {
            background-color:#92D050; color:#000;
            padding:4px 10px; border:1px solid #6aaa28;
        }
        .tbl-cbi td:last-child { text-align:right; }
        .tbl-cbi .total-row td {
            font-weight:bold; border-top:2px solid #4a8a10;
...
def carregar_csv():
    for enc in ["utf-8", "utf-8-sig", "latin-1"]:
        try:
            # Lê sem header — colunas: 0=nº, 1=nome, 2=guerra, 3=OBM, 4=horas
            df = pd.read_csv(
                URL_CSV,
                encoding=enc,
                header=None,
            ...
            # Se a primeira linha for cabeçalho textual, pula
            primeira = str(df.iloc[0, 0]).strip().lower()
            if not primeira.lstrip("-").isnumeric():
                df = df.iloc[1:].reset_index(drop=True)

            # Nomeia colunas pelo que sabemos da estrutura
            if df.shape[1] >= 5:
...
# Remove linhas sem OBM válida
df_raw = df_raw[df_raw[col_obm].astype(str).str.strip() != ""]
df_raw = df_raw[df_raw[col_obm].notna()]

# ─────────────────────────────────────────────
# AGRUPA HORAS POR OBM — ignora militares
# ─────────────────────────────────────────────
df_obm = (
...
def is_cbc(nome):
    ...
    return any(c.upper() in nome_up for c in CBC_LISTA)
...
# ─────────────────────────────────────────────
# FALLBACK com dados reais se CSV não carregou
# ─────────────────────────────────────────────
if df_cbc.empty:
    df_cbc = pd.DataFrame([
        {"OBM": "SCI",    "Horas": 1730},
...
def build_table(df_t, css, total):
    ...
    <table class="{css}">
        ...
        <tfoot>
            <tr class="total-row">
                <td><b>Total</b></td><td><b>{safe_int(total)}</b></td>
            </tr>
        </tfoot>
    </table>
    """
...
# ─────────────────────────────────────────────
# DEBUG — mostra o que foi carregado (expansível)
# ─────────────────────────────────────────────
with st.sidebar:
    st.markdown("## 🚒 CBMAM")
    st.markdown("---")
    st.markdown(f"**Registros no CSV:** {len(df_raw)}")
    st.markdown(f"**OBMs encontradas:** {len(df_obm)}")
    st.markdown(f"**OBMs CBC:** {len(df_cbc)}")
    st.markdown(f"**OBMs CBI:** {len(df_cbi)}")
    st.markdown("---")

    with st.expander("🔍 OBMs identificadas no CSV"):
        for obm in sorted(df_obm["OBM"].tolist()):
            grupo = "🟡 CBC" if is_cbc(obm) else "🟢 CBI"
            st.markdown(f"{grupo} — `{obm}`")
...
with col_g1:
    st.plotly_chart(build_chart(df_cbc, "Distribuição SEG - Capital"), use_container_width=True)
...
with col_g2:
    st.plotly_chart(build_chart(df_cbi, "Distribuição SEG - Interior"), use_container_width=True)
...
with r2:
    if not cbc_v.empty:
        st.success(f"🥇 **Maior CBC**\n\n**{cbc_v.iloc[0]['OBM']}** → {safe_int(cbc_v.iloc[0]['Horas'])}h")
    if not cbi_v.empty:
        st.success(f"🥇 **Maior CBI**\n\n**{cbi_v.iloc[0]['OBM']}** → {safe_int(cbi_v.iloc[0]['Horas'])}h")

with r3:
    if not all_v.empty:
        st.info(f"🏆 **Maior OBM Geral**\n\n**{all_v.iloc[0]['OBM']}** → {safe_int(all_v.iloc[0]['Horas'])}h")
